...
| 自营商品快照 | `GET_ETSY_STORE_SNAPSHOT` / `etsy_api_get_products` | `etsyStoreSnapshotCache` | 真实调用个人卖家 API，分页读取当前授权店铺 listings、商品详情和 receipts/发货资料；不包含 Sessions 或加购 analytics |
...
这些按钮目前调用 `dashboard.js::handleGrowthAction()`，主要行为是创建/更新 `growthCases` 与 `growthActionRuns`，并通过 `chrome.runtime.connect({ name: "etsy-agent-loop" })` 发起真实 `RUN_SKILL`：
...
它们不再只是虚拟按钮。需要注意：如果 dashboard 当前没有可注入的 Etsy 前台页面上下文，运行会进入 `needs_frontend_context`，提示用户打开对应 Etsy 页面后由右侧浮窗继续承接。
...
2. 监控事件已通过 `growthCaseId` 关联到对应案件，并在案件合同中保留 `eventIds`。
3. `growthCases` 已使用版本化合同：`contractVersion / caseId / type / evidence / taskIds / eventIds / experiments / reportIds / status / nextReviewAt / runs / runHistory`。
4. 当前剩余的是外部状态验证：使用真实 Etsy OAuth 账号确认权限 scope、receipts 分页和真实 Etsy 评论分页结构；本地 smoke 不能替代该验证。
