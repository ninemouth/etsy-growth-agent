...
1. **Etsy SEO 标题组装模型 (Title Formula)**:
   - 组装结构：`核心品类词 + 材质/颜色/尺寸 + 使用场景 + 买家身份/送礼场景 + 个性化或差异化卖点`。
   - 示例：`Personalized Satin Wedding Clutch, Bridal Evening Bag, Bridesmaid Gift Purse, Custom Name Party Handbag`。
2. **多级属性提取与对齐 (Specs Table)**:
   - 优先提取当前 Etsy 页面和竞品页面中的真实属性；只有用户明确处于寻源/供应商资料场景时，才提取供应商资料中的物理参数并准确翻译为 Etsy 标准属性（如：`Material`, `Color`, `Dimensions`, `Occasion`, `Personalization`）。
