...
const GROWTH_ACTIONS = {
  diagnose_store_growth: {
    ...
    instruction: "一键体检当前 Etsy 店铺增长瓶颈。必须先读取页面文本/API，判断店铺平台属性、经营阶段（新店冷启动/成长店/成熟店）、定位、目标人群、价格带和视觉调性；再实际访问 Etsy 站内搜索/热卖榜学习同类高排名店铺和商品；再访问 Google Search US / Google Trends US 验证站外需求。若涉及配送时效，必须实时搜索 Etsy 国际物流/目的地/承运商现状，禁止凭常识写 7-12 工作日。请按曝光、点击、加购、付款、利润、履约、评分、商品结构和欧美买家场景输出优先级行动清单；必须区分真实页面/API证据、搜索证据、AI推断和待验证假设。",
  },
  diagnose_sku_funnel: {
    label: "SKU 漏斗诊断",
    skillId: "etsy_operations_tracker",
    instruction: "诊断当前 Etsy SKU 的销售漏斗瓶颈。请区分曝光弱、点击弱、加购弱、付款弱、利润弱、履约风险和评论风险，并给出下一步实验动作。",
  },
  rewrite_listing: {
    label: "Listing 改版",
    skillId: "etsy_listing_generator",
    instruction: "基于当前 Etsy 页面或 Dashboard 选中的 SKU，生成英文 SEO 标题、主图卖点文案、详情页描述和 attributes 补齐建议。",
  },
  diagnose_visual_conversion: {
    label: "首图诊断",
    skillId: "etsy_global_shop_optimizer",
    instruction: "诊断当前商品首图和画廊视觉转化力。请检查英文卖点、中文残留、工厂图痕迹、规格表达、信任元素和竞品首图差距，并输出三种改版方向。",
  },
...
async function loadGrowthActionQueue() {
  ...
  if (!queue) return;
  const data = await new Promise((r) => chrome.storage.local.get(["growthActionRuns", "activeShopId", "etsyShops"], r));
...
async function runSkill() {
  ...
  try {
    ...
    activePort = chrome.runtime.connect({ name: "etsy-agent-loop" });
