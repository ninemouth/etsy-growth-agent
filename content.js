...
(function () {
  ...
  // ── Etsy Assistant Shadow DOM Floating UI and Overlay Chat ──
  function injectFloatingUI() {
    ...
    const GROWTH_ACTIONS = {
      diagnose_store_growth: {
        label: "店铺感知",
        short: "店铺",
        instruction: "一键感知当前 Etsy 店铺经营状态。请先判断当前页面是店铺页、商品页还是搜索/类目页；若是店铺页，重点分析店铺视觉、商品结构、转化漏斗、履约风险、评分信任、竞品对标，并输出今日必须处理的增长动作。",
      },
      diagnose_sku_funnel: {
        label: "商品分析/追踪",
        short: "商品",
        instruction: "分析并追踪当前 Etsy 商品。请读取页面标题、价格、评分、评论、主图、详情页和可见竞品信息，判断曝光、点击、加购、付款、利润、履约和评价风险，并给出是否加入 7 天增长实验。",
      },
      scan_competitor_changes: {
        label: "竞品跟踪",
        short: "竞品",
        instruction: "把当前 Etsy 页面作为竞品或竞品集合进行跟踪感知。请分析价格、促销、主图、标题关键词、评分评论、库存/履约线索和可反打机会，并输出监控建议。",
      },
      ...
      explore_platform_trends: {
        label: "平台趋势",
        short: "趋势",
        instruction: "基于当前 Etsy 搜索、类目、品牌或热卖页面扫描平台商品机会和趋势窗口。请识别热卖共性、价格带、评价门槛、英文关键词、季节性需求、Google Trends / Etsy 搜索 证据或待验证假设，并明确这不是本店扩品执行清单。",
      },
      rewrite_listing: {
        label: "Listing 改版",
        short: "改版",
        instruction: "基于当前 Etsy 商品页生成英文 SEO Listing 改版方案，包括标题、主图英文卖点、详情页描述、规格参数和风险词提醒。",
      },
      analyze_review_defects: {
        label: "评论缺陷",
        short: "评论",
        instruction: "基于当前 Etsy 商品页或可见评论信息分析欧美买家原声痛点。请归纳质量、包装、物流、尺寸、预期落差和退换货风险，并输出可落地的商品/页面改良动作。",
      },
    ...
    chatOverlay.innerHTML = `
      <div class="chat-header" id="chat-drag-handle">
        <div class="chat-title-group">
          ...
          <span class="chat-title">Etsy AI 运营智脑舱</span>
        </div>
    ...
    const describePageContext = () => {
      ...
      if (isSearchOrCatalogPage) {
        return {
          title: "已识别为 Etsy 搜索/类目页",
          subtitle: "适合发起竞品跟踪、机会扫描、价格带观察和类目扩品判断。",
          badge: "竞品/类目",
        };
      }
      return {
        title: "已进入 Etsy 运营感知模式",
    ...
    const setDockBusy = (busy, actionId = "") => {
      dockActionButtons.forEach((btn) => {
        ...
        if (busy && isActive) {
          btn.title = `正在运行：${GROWTH_ACTIONS[actionId]?.label || "Etsy 任务"}`;
        } else {
    ...
    const setActiveGrowthRun = async (run) => {
      ...
      await new Promise((r) => chrome.storage.local.set({ activeEtsyGrowthRun: run || null }, r));
    };
    ...
    const ensureMonitorTask = async (targetType = "item", shopNature = "competitor") => {
      ...
      const exists = tasks.some((task) => task.platform === "etsy" && task.target_url === window.location.href);
    ...
    const processCompetitorBaseline = async () => {
      ...
      const items = isProductPage
        ? [{
          id: pageData.url,
          title: pageData.h1 || pageData.title || "Etsy Product",
        ...
        : (pageData.productCards || []).map((card) => ({
          ...
          title: card.title || card.name || "Etsy Product",
          price: card.price || 0,
          sales: card.sales || 0,
      ...
      const shopInfo = isSellerPage || !isProductPage
        ? {
          id: pageData.url,
          name: pageData.h1 || pageData.title || "Etsy Competitor Surface",
          url: pageData.url,
          productCount: items.length,
        }
        : null;
      const response = await chrome.runtime.sendMessage({
        type: "PROCESS_ETSY_MONITOR_BASELINE",
        args: {
          items,
          shopInfo,
          shopId: "",
    ...
    const isSourcingSkillId = (skillId = "") => {
      return String(skillId).includes("etsy_sourcing_finder") || String(skillId).includes("sourcing_finder");
    };
    ...
    const renderFinalOutputMarkdown = (rawResult, skillId = "") => {
      ...
      if (shouldRenderSourcingData(skillId, res.data)) {
        ...
        res.data.filter(itemLooksLikeSourcing).forEach((item, idx) => {
          ...
          sourcingMd += `- Etsy 售价: ${ledger.target_price || "待核实"}\n`;
    ...
    const runSelectedSkill = async (instruction, growthActionId = "") => {
      ...
      termLogDiv.innerHTML = `<div class="log-line">[${new Date().toLocaleTimeString()}] ⚙️ 正在建立 Etsy AI Harness 连接通道...</div>`;
      ...
      try {
        ...
        log(`❌ 无法发起后台连接: ${contextInvalidated ? "扩展上下文已失效，请在 chrome://extensions 重新加载插件后刷新当前 Etsy 页面。" : err.message}`);
        if (contextInvalidated) {
          addMessage("assistant", "检测到扩展刚刚更新或重载，当前页面里的旧插件脚本已失效。请刷新当前 Etsy 页面后重新打开对话框，再从历史会话恢复或开启新会话。");
        }
    ...
    const saveShopBtn = shadow.getElementById("etsy-drawer-save-shop-btn");
    if (saveShopBtn) {
      saveShopBtn.addEventListener("click", async () => {
        ...
        chrome.storage.local.get(["etsyShops", "activeShopId"], (updatedData) => {
          const shopSelect = shadow.getElementById("etsy-active-shop-select");
          shopSelect.innerHTML = updatedData.etsyShops.map(s => 
            `<option value="${s.id}" ${s.id === updatedData.activeShopId ? 'selected' : ''}>🏢 ${s.name} (${s.shopId || s.clientId || s.id})</option>`
          ).join('');
          
          const listDiv = shadow.getElementById("etsy-drawer-shops-list");
          listDiv.innerHTML = updatedData.etsyShops.map(s => `
            <div style="display:flex; justify-content:space-between; align-items:center; padding:5px 8px; background:var(--btn-bg); border-radius:4px; font-size:11px; border:1px solid ${s.id === updatedData.activeShopId ? '#005bff' : 'transparent'};">
              <span style="color:${s.id === updatedData.activeShopId ? '#005bff' : 'var(--text-color)'}; font-weight:${s.id === updatedData.activeShopId ? 'bold' : 'normal'};">🏢 ${s.name} (${s.shopId || s.clientId || s.id})</span>
              <span class="etsy-drawer-delete-shop" data-id="${s.id}" style="color:#ef4444; cursor:pointer; font-weight:bold; font-size:12px; padding:0 4px;" title="删除">×</span>
            </div>
          `).join('');
          
          listDiv.querySelectorAll(".etsy-drawer-delete-shop").forEach(delBtn => {
            delBtn.addEventListener("click", async (ev) => {
              ev.stopPropagation();
              const shopId = delBtn.getAttribute("data-id");
              if (confirm("确定要删除此店铺的 API 绑定吗？")) {
                const innerStorage = await new Promise(res => chrome.storage.local.get(["etsyShops", "activeShopId"], res));
                let innerShops = innerStorage.etsyShops || [];
                ...
                await new Promise(res => chrome.storage.local.set({ etsyShops: innerShops, activeShopId: nextActive }, res));
                alert("店铺删除成功！");
                settingsDrawer.classList.add("hidden");
              }
            });
    ...
    settingsBtn.addEventListener("click", () => {
      ...
      chrome.storage.local.get([
        ...
        const renderDrawerShops = () => {
          const currentShops = data.etsyShops || [];
          ...
          if (currentShops.length === 0) {
            ...
            listDiv.innerHTML = currentShops.map(s => `
              <div style="display:flex; justify-content:space-between; align-items:center; padding:5px 8px; background:var(--btn-bg); border-radius:4px; font-size:11px; border:1px solid ${s.id === currentActive ? '#005bff' : 'transparent'};">
                <span style="color:${s.id === currentActive ? '#005bff' : 'var(--text-color)'}; font-weight:${s.id === currentActive ? 'bold' : 'normal'};">🏢 ${s.name} (${s.shopId || s.clientId || s.id})</span>
                <span class="etsy-drawer-delete-shop" data-id="${s.id}" style="color:#ef4444; cursor:pointer; font-weight:bold; font-size:12px; padding:0 4px;" title="删除">×</span>
              </div>
            `).join('');
            ...
            listDiv.querySelectorAll(".etsy-drawer-delete-shop").forEach(delBtn => {
              delBtn.addEventListener("click", async (e) => {
                ...
                const shopId = delBtn.getAttribute("data-id");
                if (confirm("确定要删除此店铺的 API 绑定吗？")) {
                  data.etsyShops = data.etsyShops.filter(s => s.id !== shopId);
                  if (data.activeShopId === shopId) {
                    data.activeShopId = data.etsyShops.length > 0 ? data.etsyShops[0].id : "";
                  }
                  if (data.etsyShops.length > 0 && !data.etsyShops.some(s => s.isDefault)) {
                    data.etsyShops[0].isDefault = true;
                  }
                  await new Promise(r => chrome.storage.local.set({
                  ...
                  renderDrawerShops();
                  if (data.etsyShops.length === 0) {
                    shopSelect.innerHTML = `<option value="">-- 未绑定店铺 --</option>`;
                  } else {
                    shopSelect.innerHTML = data.etsyShops.map(s => 
        ...
        shadow.getElementById("etsy-target-margin").value = data.etsyTargetMargin || '20';
