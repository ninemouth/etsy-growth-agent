...
function editorDom(markup = "") {
  return new JSDOM(`<form id="editor">${markup}<button id="save" type="submit">Save draft</button><button id="publish" type="button">Publish</button></form>`, { url:"https://www.etsy.com/your/shops/TargetShop/listing/draft-123" });
}

const executionBinding = { etsyShopRef:"targetshop" };
...
const inspection = writer.inspectEditor(draft, { documentImpl:dom.window.document, locationHref:dom.window.location.href, executionBinding });
assert.equal(inspection.ready, true);
assert.equal(inspection.etsyShopRef, "targetshop");
assert.equal(inspection.approvedRequiredValuesMatch, false);
const result = writer.applyApprovedDraft(draft, { documentImpl:dom.window.document, locationHref:dom.window.location.href, EventImpl:dom.window.Event, executionBinding });
...
assert.equal(result.selectorSetVersion, "etsy-listing-editor-selectors.2026-09-04.v2");
assert.equal(result.etsyShopRef, "targetshop");
assert.equal(writer.inspectEditor(draft, { documentImpl:dom.window.document, locationHref:dom.window.location.href, executionBinding }).approvedRequiredValuesMatch, true);
...
assert.throws(() => writer.applyApprovedDraft(draft, { documentImpl:wrongPage.window.document, locationHref:"https://www.etsy.com/account/security", EventImpl:wrongPage.window.Event, executionBinding }), /not an allowed listing editor route/);
...
assert.throws(() => writer.applyApprovedDraft(draft, { documentImpl:missingPrice.window.document, locationHref:missingPrice.window.location.href, EventImpl:missingPrice.window.Event, executionBinding }), /price.*No fields were changed/);
...
assert.throws(() => writer.applyApprovedDraft(draft, { documentImpl:readOnly.window.document, locationHref:readOnly.window.location.href, EventImpl:readOnly.window.Event, executionBinding }), /title.*No fields were changed/);
assert.throws(() => writer.inspectEditor(draft, { documentImpl:dom.window.document, locationHref:"https://www.etsy.com/your/shops/me/listing/draft-123", executionBinding }), /exact Etsy shop/);
assert.throws(() => writer.inspectEditor(draft, { documentImpl:dom.window.document, locationHref:dom.window.location.href, executionBinding:{ etsyShopRef:"another-shop" } }), /another shop/);
...
assert.match(content, /INSPECT_APPROVED_ETSY_DRAFT/);
