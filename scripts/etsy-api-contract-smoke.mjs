...
assert.match(source, /etsyGetAllProductListings[\s\S]*pagesFetched[\s\S]*productsComplete/);
assert.match(source, /etsyGetReceiptWindow[\s\S]*receiptCoverage[\s\S]*receiptsComplete/);
...
assert.match(backgroundSource, /growthCaseId: task\.growthCaseId/);
