...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === "AUTH_EXTENSION_REPORT") {
    reportBrowserExtensionInstallationStatus()
      .then((result) => sendResponse({ ok: result.ok, result, error: result.ok ? "" : result.error, errorCode: result.errorCode }))
      .catch((err) => sendResponse({ ok: false, error: err.message, errorCode: err.code || "EXTENSION_INSTALLATION_REPORT_FAILED" }));
    return true;
  }
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  if (alarm.name === DEVICE_AUTH_ALARM) {
    try {
      ...
      if (result.status === "authorized") await reportBrowserExtensionInstallationStatus();
