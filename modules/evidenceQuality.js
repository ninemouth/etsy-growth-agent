function levelForSource(sourceType = "") {
  ...
  if (["etsy_api", "supplier_page", "official_policy", "official_regulation"].includes(normalized)) return "A";
  if (["etsy_search", "google_search", "google_trends"].includes(normalized)) return "B";
  if (["page_dom", "screenshot_visual", "sourcing_search", "review_dom"].includes(normalized)) return "C";
  return "D";
}
...
function flattenLedgers(output = {}) {
  ...
  (Array.isArray(output.data) ? output.data : []).forEach((item, index) => {
    (Array.isArray(item?.evidence_ledger) ? item.evidence_ledger : []).forEach((entry) =>
      addLedger(entry, item?.title || item?.name || item?.opportunity_id || item?.action_id || `data-${index + 1}`)
    );
  });
  (Array.isArray(output.competitor_benchmarks) ? output.competitor_benchmarks : []).forEach((item, index) => {
    (Array.isArray(item?.evidence_ledger) ? item.evidence_ledger : []).forEach((entry) =>
      addLedger(entry, item?.competitor_name || item?.shop_name || `competitor-${index + 1}`)
    );
  });
  return ledgers;
}
...
export function summarizeEvidenceQuality({ output = {}, pageContext = {}, researchScope = {} } = {}) {
  ...
  const hasApi = sourceTypes.has("etsy_api") || Boolean(pageContext.etsyApiSnapshot || pageContext.apiSnapshot);
  const hasSearch = ["etsy_search", "google_search", "google_trends"].some((type) => sourceTypes.has(type));
  const hasPage = ["page_dom", "screenshot_visual", "review_dom"].some((type) => sourceTypes.has(type)) ||
    Boolean(pageContext.url || pageContext.screenshot);
  ...
  return {
    ...
    data_freshness: freshness ? { etsy_api_synced_at: freshness } : {},
    ...
    personal_api_boundary: "Etsy API evidence can only describe the authorized seller account; competitor and platform conclusions must use public-page/search evidence.",
