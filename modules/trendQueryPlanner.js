...
export function classifyGoogleTrendsEvidence(result = {}) {
  ...
  const shellLoaded = /google trends|trends explore|trend_shell/i.test(text) ||
...
export function getTrendQueryGuardError({ skillId = "", toolName = "", toolArgs = {}, toolHistory = [] } = {}) {
  ...
  if (!normalizedQuery) {
    return {
      ...
      error: "Google Trends 查询缺少明确关键词。请从关键词漏斗的聚焦词中选择一个明确查询，不要提交空查询。",
    };
  }
  ...
  if (duplicate) {
    return {
      type: "trend_query_guard",
      error: `趋势词“${query}”本轮已经查询过，不允许重复打开相同页面。请退宽一个语义层级或切换 Etsy/Google 已发现的英语同义词。`,
  ...
  if (attempts.length >= MAX_GOOGLE_TRENDS_QUERY_ATTEMPTS) {
    return {
      type: "trend_query_guard",
      error: "Google Trends 关键词恢复已达到 3 次上限。停止继续搜索，把该来源标记为数据不足，并使用 Etsy/Google 的真实证据完成机会判断。",
      attemptedQueries: attempts.map((attempt) => attempt.queryUsed || attempt.requestedQuery),
    };
  }
  return null;
...
export function getTrendQueryRefinementState(skillId = "", toolHistory = []) {
  ...
  return {
    ...
    message: firstRecovery
      ? `当前趋势词“${last.queryUsed || last.requestedQuery}”页面已加载但数据不足。请立即退宽一个语义层级：删除产地、年份、用途等组合修饰，改用 1-2 个词的英语品类头词；新词必须来自本轮 Etsy/Google 已发现词族，并标记为 parent_proxy，不能直接证明原始细分品类增长。`
      : `第二个趋势词“${last.queryUsed || last.requestedQuery}”仍缺少数据。请切换到另一个英语同义词族或相邻需求表达，不要重复旧词；这是最后一次恢复查询，并必须标记 exact / parent_proxy / adjacent_proxy 的范围关系。`,
  };
}
