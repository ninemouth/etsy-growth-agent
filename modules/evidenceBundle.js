...
function compactProductCard(card = {}) {
  return {
    ...
    url: card.href || card.url || card.listingUrl || card.link || "",
    imageSrc: card.imageSrc || card.imageUrl || card.img || card.candidate_image_url || "",
  };
}

function compactPageData(pageData = {}) {
  if (!isObject(pageData)) return {};
  const visibleText = String(pageData.visibleText || pageData.text || pageData.bodyText || "");
  return {
    url: pageData.url || pageData.canonicalUrl || pageData.currentPageUrl || "",
    title: pageData.title || pageData.h1 || "",
    pageType: pageData.pageType || "",
    shopName: pageData.shopName || "",
    visibleTextLength: visibleText.length,
    visibleTextSnippet: truncateText(visibleText, 700),
    ...
    productCards: (Array.isArray(pageData.productCards) ? pageData.productCards : []).slice(0, 10).map(compactProductCard),
...
function compactPageRecord(page = {}) {
  return {
    ...
    visibleTextSnippet: truncateText(page.visibleTextSnippet || page.visibleText || "", 360),
    productCardsVisible: page.productCardsVisible,
    sortLabel: page.sortLabel || "",
    pagination: page.pagination || {},
    productCards: (Array.isArray(page.productCards) ? page.productCards : []).slice(0, 10).map(compactProductCard),
...
function compactResult(result = {}) {
  ...
  const compacted = {
    ...
    readinessElapsedMs: result.readinessElapsedMs,
    stableReads: result.stableReads,
...
export function buildEvidenceBundle({
  ...
  const bundle = {
    ...
    api_boundary: "Etsy personal seller API only covers the authorized seller account. Competitor/platform evidence must come from public pages and public search tools.",
  ...
  if (JSON.stringify(bundle).length <= TOOL_RESULT_LIMIT * 4) return bundle;
  return {
    ...bundle,
